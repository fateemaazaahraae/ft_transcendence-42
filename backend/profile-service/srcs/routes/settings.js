...
export function settingsRoutes(fastify) {
    ...
    //     return rep.status(res.status).send(data);
    // });
    // fastify.put("/settings/:id/upload", async (req, reply) => {
    //     const data = await req.file();
    //     if (!data) return reply.status(400).send({ error: "No file sent" });
    ...
    //     });
    //     const res = await fetch(`http://auth-service:3000/users/${req.params.id}/upload`, {
    //         method: "PUT",
    //         body: form,
    //         headers: form.getHeaders(),
    //     }
